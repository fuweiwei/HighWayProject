...
import com.ty.highway.highwaysystem.ui.fragment.check.TaskTabFragment;
...
public class TaskListActivity extends BaseActivity implements View.OnClickListener{
    ...
    public void init(){
        ...
        if(!TextUtils.isEmpty(mTunenlId)){
            setTitleText(DBBTunnelDao.getInstance(mContext).getTunnelById(mTunenlId) + "土建检查-任务列表");
        }else{
            setTitleText("土建检查-任务列表");
        }
        initView();
    ...
    public void initView(){
        ...
        for (int i = 0; i < mTabList.size(); i++) {
            TaskTabFragment f1 = new TaskTabFragment();
